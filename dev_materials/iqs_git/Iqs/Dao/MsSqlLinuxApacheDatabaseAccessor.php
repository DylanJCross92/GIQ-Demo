    ...
    class MsSqlLinuxApacheDatabaseAccessor extends ADatabaseAccessor
    {
        ...
        public function updateZipWhiteListValues(array $whiteListValues)
        {
            ...
            foreach( $whiteListValues as $whiteListItem ) {
                ...
                $sqlCheck = odbc_prepare($dbcon,"SELECT TOP 1 * FROM [ZipCodeWhiteList] WHERE [WhiteListId] = ?;");
                $whiteListItemCheck = array($whiteListItem[3]);
                $resultCheck = odbc_execute($sqlCheck,$whiteListItemCheck);
                if(!$resultCheck){
                    odbc_close($dbcon);
                    throw new IqsException(InformationCode::$SYS_UNABLE_TO_ACCESS_DATABASE);
        ...
        public function replaceZipWhiteListValues(array $whiteListValues)
        {
            $dbcon = $this->connectToDatabase();

            $sqlCheck = odbc_prepare($dbcon,"SELECT TOP 1 * FROM [ZipCodeWhiteList]");

            $resultCheck = odbc_execute($sqlCheck);
            ...
            $sql = odbc_prepare($dbcon, "TRUNCATE TABLE [ZipCodeWhiteList]");
        ...
        public function addBlockCodesValues(array $blockCodesValues)
        {
            ...
            foreach( $blockCodesValues as $BlockCodeItem ) {
                ...
                if(!$resultCheck){
                    ...
                    $rows = array();
                    //This will check if block code already exists
                    while($myRow = odbc_fetch_array($sqlCheck)){
                        $rows[] = $myRow;
                    }
                    //Only insert block code if it does not already exist.
                    if(count($rows)==0){
        ...
        /**
         * bulkUpdateBlockCodesValues
         * Will insert new block code if unique and update message if duplicate
         */
        public function bulkUpdateBlockCodesValues(array $blockCodesValues)

        {
            ...
            foreach( $blockCodesValues as $blockCodeItem ) {
                ...
                //This will return true if the block code exists or not.
                $resultCheck = odbc_execute($sqlCheck,array($blockCodeItem[0]));
                if(!$resultCheck){
                    odbc_close($dbcon);
                    throw new IqsException(InformationCode::$SYS_UNABLE_TO_ACCESS_DATABASE);
                    ...
                    //if the code does not exist, insert
                    if(count($rows)==0) {
                        $sql = odbc_prepare($dbcon, "INSERT INTO [BlockCodes] ([BlockCode], [BlockText], [DateTime]) VALUES (?,?,GETDATE())");
                        $result = odbc_execute($sql, $blockCodeItem);
                        if (!$result) {
                        ...
                        //if the code  exists, update it
                        $sql = odbc_prepare($dbcon, "UPDATE [BlockCodes] SET [BlockText]=? WHERE [BlockCode]=?");
                        $result = odbc_execute($sql, array($blockCodeItem[1], $blockCodeItem[0]));
                        if (!$result) {
                            odbc_close($dbcon);
                            throw new IqsException(InformationCode::$SYS_UNABLE_TO_ACCESS_DATABASE);
                        }
        ...
        public function replaceBlockCodesValues(array $blockCodesValues)

        {
            $dbcon = $this->connectToDatabase();

            $sqlCheck = odbc_prepare($dbcon,"SELECT TOP 1 * FROM [BlockCodes]");
            ...
            $sql = odbc_prepare($dbcon, "TRUNCATE TABLE [BlockCodes]");
        ...
        public function updateConfElementValue($confSection, $confElement, $confValue)
        {
            ...
            $sqlCheck = odbc_prepare($dbcon,"SELECT TOP 1 * FROM [".FieldKeys::$IQS_DATABASE_CONF_TABLE."] 
                WHERE [".FieldKeys::$IQS_DATABASE_CONF_SECTION."]=? 
                AND [".FieldKeys::$IQS_DATABASE_CONF_ELEMENT."]=?");

            //This will return true if the block code exists or not.
            $resultCheck = odbc_execute($sqlCheck,array($confSection, $confElement));
            if(!$resultCheck){
                odbc_close($dbcon);
                throw new IqsException(InformationCode::$SYS_UNABLE_TO_ACCESS_DATABASE);
            }else{
