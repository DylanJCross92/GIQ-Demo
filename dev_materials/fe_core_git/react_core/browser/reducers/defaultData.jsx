...
import { setVerAddress, setAddresses, setEnteredAddress, setBeginQuote, setSessionId, setSaveQuote, setQuoteId, setAddVal, setMaskedSSN} from './session.jsx'
import { setIsSaving, setThankYou, setIncPage, setIsValidating, dispatchSelectedHomeStyle, dispatchSelectedGarageType, dispatchSelectedRoofStyle, dispatchSelectedRoofType, setLoadQuote } from './popups.jsx'
    ...
    return beginQuoteSession(session.sessionId || getSessionCookie(config.Product_State) , beginQuoteDataArray)
        ...
        .then(data => {
            if(data.exception != null) {
                ...
                dispatch(setQuoteId(data.responseDataPayload.QuoteId.split('-')[1]))
    ...
    return getSessionStorageData(sessionId)
        ...
        .then(data => {
            ...
            if(!data.responseDataPayload.SessionStorageData){
                ...
                if(quoteData.Insured1SSN){
                    let ssn = quoteData.Insured1SSN
                    let maskedSSN = `***-**-${ssn.slice(-4)}`
                    dispatch(setMaskedSSN(maskedSSN))

                }


                dispatch(setDataFields(quoteData))
                return dispatch(setLoadQuote(false))
            }
        })
        .catch(err => {
            console.log(err)
