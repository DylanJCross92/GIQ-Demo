...
$(function(){ 
    ...
    $(document).on("click", ".replace-csv-zip-whitelist", function(e){
        e.preventDefault();

        $("form[name='replace_zip_whitelist_csv'] [name='csv']").trigger("click");
        $(".import-error").hide();
    });


    $(document).on("change", "form[name='replace_zip_whitelist_csv'] [name='csv']", function(e){

        e.preventDefault();

        var form = $(this).closest("form")[0];
        var formData = new FormData(form);
        formData.append('update', 'replace')
        var $table = $(".table.zipcode-whitelisting");
	...
	$(document).on("change", "form[name='blockcodes_csv'] [name='csv']", function(e){
        // console.log('event in change', e.target);
    ...
    $(document).on("click", ".replace-csv-blockcodes", function(e){
        e.preventDefault();
        $("form[name='replace_blockcodes_csv'] [name='csv']").trigger("click");
        $(".import-error").hide();
    });


    $(document).on("change", "form[name='replace_blockcodes_csv'] [name='csv']", function(e){
        console.log('event in change', e.target);
...
$(function(){
	...
	/* ON CLICK - Show Configuration Popup */
    $(document).on("click", "[data-add-row='true'][data-table-name='config']", function(){

        $.ajax({
            url: "ajax/view/popups/add-config.php",
            dataType: "html",
            type: "POST",
            success: function(html) {
                $(".overlay-wrapper .overlay-container").html(html);
    ...
    $(document).on("click", ".overlay-container [name='add_config'] input[type='submit']", function(e){
    	console.log(e)
        e.preventDefault();
        var $form = $(this).closest("form");
        var $serializedData = $form.serialize();
    ...
    $(document).on("click", ".config-submit", function(e){
        ...
        $.ajax({
            url: "ajax/insert/insertConfiguration.php",
            dataType: "html",
            type: "POST",
            data: $data,
            success: function(html) {
                ...
                $(".overlay-wrapper").fadeOut("fast");
                $loader.hide();
				$("a[href='settings/configuration']")[0].click();
            }
        });
    ...
    //Config deletes
    $(document).on("click", ".configuration-content .delete", function (e){
        e.stopPropagation()
    ...
    $(window).click(function() {
        ...
        var $rowDelete = $(".configuration-content .row.confirm-delete");
    ...
    $(document).on("click", ".configuration-content .row.confirm-delete .delete", function(e){
        e.stopPropagation();
        var $confSection = $(this).data("conf-sec");
        var $confElement = $(this).data("conf-element");
        var $loader = $(this).closest(".overlay-container").find(".loading-wrapper");

        $loader.show();

        $.ajax({
            url: "ajax/delete/deleteConfiguration.php",
            dataType: "html",
            type: "POST",
            data: "confSection="+$confSection+"&confElement="+$confElement,
            success: function(html) {
