...
{
	$success = $Iqs->addZipWhiteListValues($newWhiteListItemsArray) ? $Iqs->addZipWhiteListValues($newWhiteListItemsArray) : false;
	if (!$success){
        $response = array(
            "error" => true
        );

        $response["message"] = "Failure to insert rows";
        require ROOT."/app/views/parts/settings/whitelist.import.error.php";
